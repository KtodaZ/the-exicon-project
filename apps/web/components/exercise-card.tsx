...
export function ExerciseCard({ exercise, onTagClick, className }: ExerciseCardProps) {
  const { name, description, tags, urlSlug, difficulty, image_url } = exercise;
  ...
  const handleAnchorClick = (e: React.MouseEvent) => {
    ...
    console.log('Target URL:', `/exicon/${urlSlug}`);
    console.log('Event:', e);
    // Don't prevent default - let the anchor work normally
  };

  const targetUrl = `/exicon/${urlSlug}`;
  console.log('Rendering card with URL:', targetUrl);
          ...
          <div className="aspect-video bg-gray-200 dark:bg-gray-800 rounded-md mb-4 flex items-center justify-center overflow-hidden">
            {image_url ? (
              ...
              <svg 
                ...
                className="h-12 w-12 text-gray-400 dark:text-gray-600 pointer-events-none" 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
