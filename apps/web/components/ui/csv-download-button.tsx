...
export default function CSVDownloadButton({
  ...
  const downloadCSV = async (type: 'filtered' | 'all') => {
    ...
    try {
      ...
      params.append('includeUserDrafts', includeUserDrafts.toString());
      params.append('type', type);

      const response = await fetch(`/api/exercises/download-csv?${params.toString()}`);
      ...
      let filename = `exicon-exercises-${type}-${new Date().toISOString().split('T')[0]}.csv`;
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50">
          <div className="py-1">
            {hasFiltersApplied && (
              ...
              >
                <div className="flex flex-col">
                  <span className="font-medium">Download Current Results</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    Export filtered/searched exercises
                  </span>
                </div>
              </button>
            )}
            ...
            >
              <div className="flex flex-col">
                <span className="font-medium">Download All Exercises</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">
